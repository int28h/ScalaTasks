...
object Solution{
  def main(args: Array[String]): Unit = {
    ...
    actual = stdin.readLine.split(" ").map(_.trim.toInt)

    var expected = new Array[Int](3)
    expected = stdin.readLine.split(" ").map(_.trim.toInt)

    var fine = 0
    
    if (expected(2) == actual(2)) if (expected(1) < actual(1)) fine = (actual(1) - expected(1)) * 500
    else if ((expected(1) == actual(1)) && (expected(0) < actual(0))) fine = (actual(0) - expected(0)) * 15
    else if (expected(2) < actual(2)) fine = 10000
